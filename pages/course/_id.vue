<template>
 ...
 <script>
 import courseApi from '../../api/index/course';
 import orders from '../../api/index/orders';
 export default {
   data() {
     return {
       courseWebVo: {},
     ...
     showInfo() {
       courseApi.getCourseInfo(this.$route.params.id).
         then(res => {
           ...
           this.chapterVoList = res.data.chapterVoList
           this.isBuy = res.data.isBuy
         })
     },
     createOrder() {
       orders.createOrder(this.$route.params.id).then(res => {
